package signal.chirp is
   ...
   type Chirp_Type is abstract new Signal_Type with
      record
         startfreq : Integer ;
         endfreq : Integer ;
   ...
   type LinearChirp is new Chirp_Type and wave.Generator and wave.WGenerator with null record ;
   function Value( g : in out LinearChirp ; arg : float ) return float ;
   procedure Initialize( g : in out LinearChirp ; w : wave.Wave_Type );
   
   type ExponentialChirp is new Chirp_Type and wave.Generator and wave.WGenerator with 
      record
         lnchirpiness : float ;
      end record ;
