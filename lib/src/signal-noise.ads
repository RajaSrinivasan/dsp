package signal.noise is
   ...
   type UncorrelatedNoise_Type is new NonPeriodic_Signal_Type and wave.Generator with null record ;
   function Value( g : in out UncorrelatedNoise_Type ; arg : float ) return float ;

   type GaussianNoise_Type is new NonPeriodic_Signal_Type and wave.Generator with null record ;
   ...
   type BrownianNoise_Type is new NonPeriodic_Signal_Type and wave.Generator with 
      record
         cumsum : float := 0.0 ;
      end record ;
