...
package body sndfile is
   ...
   with Convention => C_Pass_By_Copy;  -- ../sndfile.h:376
   ...
   with Convention => C_Pass_By_Copy;  -- ../sndfile.h:397
   ...
   type SF_EMBED_FILE_INFO is record
     ...
     with Convention => C_Pass_By_Copy;  -- ../sndfile.h:406
   

   function sf_open
     (path : Interfaces.C.Strings.chars_ptr;
      mode : int;
      sfinfo : access SF_INFO) return SNDFILE  -- ../sndfile.h:562
   with Import => True, 
        Convention => C, 
   ...
   function sf_open_fd
     (fd : int;
      ...
      close_desc : int) return SNDFILE  -- ../sndfile.h:577
   with Import => True, 
        Convention => C, 
     External_Name => "sf_open_fd";
   
   function sf_close (the_sndfile : SNDFILE) return int  -- ../sndfile.h:727
   with Import => True, 
        Convention => C, 
   ...
   function sf_seek
     (the_sndfile : SNDFILE;
      frames : sf_count_t;
      whence : int) return sf_count_t  -- ../sndfile.h:641
   with Import => True, 
        Convention => C, 
     External_Name => "sf_seek";
   
   function sf_error (the_sndfile : SNDFILE) return int  -- ../sndfile.h:586
   with Import => True, 
        Convention => C, 
     External_Name => "sf_error";
   
   
   function sf_strerror (the_sndfile : SNDFILE) return Interfaces.C.Strings.chars_ptr  -- ../sndfile.h:593
   with Import => True, 
        Convention => C, 
        External_Name => "sf_strerror";

   function sf_error_number (errnum : int) return Interfaces.C.Strings.chars_ptr  -- ../sndfile.h:601
   with Import => True, 
        Convention => C, 
   ...
   function sf_read_float
     (the_sndfile : SNDFILE;
      ptr : Address ;
      items : sf_count_t) return sf_count_t  -- ../sndfile.h:715
   with Import => True, 
        Convention => C, 
   ...
   function sf_write_float
     (the_sndfile : SNDFILE;
      ptr : Address ;
      items : sf_count_t) return sf_count_t  -- ../sndfile.h:716
   with Import => True, 
        Convention => C, 
   ...
   begin      
      Put("Frames : "); Put( info.frames ); New_Line ;
      Put("Sample Rate : "); Put( info.samplerate ) ; New_LIne ;
      Put("Audio Length : "); Put( float(info.frames) / float(info.samplerate) ); Put(" seconds"); New_Line ;
      Put("Channels : " ); Put( info.channels) ;
      Put(" format : " ); Put( info.format ) ;
      Put(" sections : "); Put( info.sections ) ;
      Put(" seekable : "); Put( info.seekable ) ;
      New_Line ;
   end Show ;
   ...
   begin
      ...
      if Verbose
      then
         Put("File "); Put(fn); Put(" opened"); New_Line ;
         Show( result.info ) ;
   ...
   begin
      --if Verbose
        --then
         Put("Creating "); Put(fn); Put(" with "); New_Line ;
         Show(info) ;
      --end if ;
   ...
   function Info( f : file_Type ) return SF_INFO is
      use System ;
   begin
      if f.file = SNDFILE(System.Null_Address)
      then
   ...
   function Read( f : file_type ; from : float ; span : float ) return AudioFragment_Type is
      ...
      CURFRAME : integer ;
   begin
      if f.info.seekable = 0
      then
         raise Program_Error with "Read audio not seekable" ;
      end if ;
      start_frame := sf_count_t( from * float(f.info.samplerate) ) ;
      if start_frame > f.info.frames
      then
         raise Program_Error with "Read startframe" ;
      end if ;
      need_frames := 1 + sf_count_t( span * float(f.info.samplerate) ) ;
      if start_frame + need_frames > f.info.frames
      then
         raise Program_Error with "Read frames" ;
      end if ;
      if Verbose
      then
         Put("Reading "); Put( need_frames ) ; put(" frames from "); Put( start_frame ) ; New_Line ;
      end if ;
      result.ch1 := new Ada.Numerics.Real_Arrays.Real_Vector(1..integer(need_frames)) ;
      if f.info.channels > 1
      then
         result.ch2 := new Ada.Numerics.Real_Arrays.Real_Vector(1..integer(need_frames)) ;
      end if ;
      ...
      while need_frames > 0
      loop
         readframes := sf_read_float(f.file,BLOCK(BLOCK'first)'Address,sf_count_t(BLOCKFRAMES)); 
         if readframes < 1
         then
            exit ;
         end if ;
         Put("Read "); Put(readframes) ; Put(" frames. Current frame "); Put(CURFRAME); New_Line;
         for frame in 1..readframes
         loop
   ...
   begin
      ...
      loop
         ...
         if writecount /= 2
         then
            Put("Wrote "); Put(writecount); Put(" frames"); New_Line ;
            ReportError( f.file ) ;
         end if ;
      end loop ;
