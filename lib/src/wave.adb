...
package body wave is
   ...
   function Create( sample_rate : integer ;
                    start : float ;
                    span : float ;
                    default : float := 0.0 ) return Wave_Type is
   ...
   begin
      ...
      for r in 1 + result.samples'first .. result.samples'last
      loop
   ...
   begin
      -- if Verbose
      -- then
         Put("Creating wave with "); Put(count); Put(" samples"); New_Line ;
         Put("Initial data length "); Put( values'length ) ; New_Line ;
      -- end if;
   ...
   function CreateComplex( sample_rate : integer ;
                           start : float ;
                           span : float ;
                           default : Ada.Numerics.Complex_Types.Complex := ( Re => 0.0, Im => 0.0 ) ) return Wave_Type is
   ...
   begin
      ...
      for r in 1 + result.csamples'first .. result.csamples'last 
      loop 
         result.Xs(r) := result.Xs(r-1) + delta_t ;
   ...
   function Create( like : Wave_Type ;
                    length : Integer := 0 ) return Wave_Type is
   ...
   begin
      ...
      if resultlen > like.Xs'length
      then
         bigger := true ;
         result.Xs( like.Xs'range ) := like.Xs.all ;
         for xp in like.Xs'Last+1 .. result.Xs'Last 
         loop
      ...
      when true =>
         ...
         then
            ...
            result.samples(like.samples'Last+1..result.samples'Last) := ( others => 0.0 ) ;
         else
      ...
      when false =>
         ...
         then
            ...
            result.csamples(like.csamples'Last+1..result.csamples'Last) := ( others => (Re=>0.0,Im=>0.0) ) ;
         else
   ...
   begin
      ...
      fp := 1 ;
      while fp <= result.Xs'Length
      loop
         if abs(result.Xs(fp) - Right.Xs(1)) < epsilon
         then
            for rp in right.Xs'range
            loop
               case result.real is
                  when true =>
                     result.samples(fp + rp  -1 ) := result.samples(fp + rp - 1 ) + right.samples(rp) ;
                  when false =>
                     result.csamples(fp + rp  -1 ) := result.csamples(fp + rp - 1 ) + right.csamples(rp) ;                     
               end case ;
            end loop ;
            return result ;
         end if ;
   ...
   begin
      ...
      for rx in w.Xs'range
      loop
         case w.real is
            when true =>
               if temp < w.samples(rx)
               then
                  temp := w.samples(rx) ;
               end if ;
            when false =>
               if temp < Modulus(w.csamples(rx))
               then
                  temp := Modulus(w.csamples(rx)) ;
               end if ;
         end case ;
   ...
   function Apply( w : windows.Window_Type'Class ; wi : Wave_Type ; offset : integer ) return Wave_Type is 
      result : Wave_Type ;
   begin
      result := Wave.Create( wi.sample_rate ,wi.Xs(offset+wi.Xs'first), (float(w.factors'Length-1)) * wi.deltax );
      for valp in result.Xs'range
      loop
         result.samples(valp) := wi.samples(wi.Samples'first + offset + valp ) * w.factors(valp) ;
      end loop ;
      return result ;
   end Apply ;
